      ...
      >
        <template #default="{ row, $index }">
          ...
          <div class="adjust-ratio-cell">
            <!-- 非编辑模式：显示编辑按钮和调整值 -->
            <template v-if="!editingStates[$index]">
              <!-- 编辑调整申报比例按钮 -->
              <div
                class="edit-btn"
                @click="!isEditingDisabled && enterEditMode($index)"
                :class="{ 'edit-btn-disabled': isEditingDisabled }"
                :title="isEditingDisabled ? '申报进行中，不可编辑' : '点击编辑'"
              >
            ...
            <template v-else>
              ...
              <el-input-number
                ...
                @change="(value) => handleRatioInput($index, value)"
                @blur="handleRatioBlur($index)"
              >
              </el-input-number>
              <!-- 确认调整申报比例按钮 -->
              <div class="confirm-btn" @click="exitEditMode($index)">
...
const props = defineProps({
  ...
  ratios: {
    type: Array,
    default: () => new Array(24).fill(null),
  },
  // 编辑按钮禁用状态
  isEditingDisabled: {
    type: Boolean,
...
// Emits定义
const emit = defineEmits(["update:ratios", "cell-click"]);
...
watch(
  () => props.tableData,
  (newData) => {
    editingStates.value = new Array(newData.length).fill(false);
  },
  { immediate: true },
);
...
const exitEditMode = (index) => {
  ...
  editingStates.value[index] = false;
};
...
const cellStyle = ({ row, column, rowIndex, columnIndex }) => {
  ...
  // 突出显示可编辑列
  if (column.property === "adjustedRatio") {
  ...
  if (rowIndex % 2 === 0) {
    ...
    style.backgroundColor = style.backgroundColor || "#fafafa";
  }
...
// 处理比例输入
const handleRatioInput = (index, value) => {
  const newRatios = [...props.ratios];
  const originalRatio = props.tableData[index]?.ratio; // 原始申报比例

  // 处理NaN、null、undefined和空字符串情况
  if (value === "" || value === null || value === undefined || isNaN(value)) {
    // 清空输入框或无效输入时恢复为原始比例
    newRatios[index] = originalRatio != null ? originalRatio : 1;
  } else {
    const num = parseFloat(value);
    if (!isNaN(num)) {
      // 限制在0-2之间（0-200%）
      newRatios[index] = Math.max(0, Math.min(2, num));
    } else {
      // 如果解析失败，恢复为原始比例
      newRatios[index] = originalRatio != null ? originalRatio : 1;
    }
  ...
  emit("update:ratios", newRatios);
};

// 处理比例输入框失去焦点
const handleRatioBlur = (index) => {
  const currentValue = props.ratios[index];
  const originalRatio = props.tableData[index]?.ratio; // 原始申报比例

  // 确定基准值：如果当前值为null/undefined，使用原始比例；如果原始比例也为null，使用1
  const baseValue =
    currentValue == null
      ? originalRatio != null
        ? originalRatio
  ...
  // 确保比例在合理范围内
  const clampedValue = Math.max(0, Math.min(2, baseValue));
  if (clampedValue !== currentValue) {
    const newRatios = [...props.ratios];
    newRatios[index] = clampedValue;
    emit("update:ratios", newRatios);

    // 显示提示
    ElMessage.warning("比例已自动调整为0-200%之间");
  }
};
...
// 重置所有调整比例
const resetAllRatios = () => {
  ElMessageBox.confirm(
    "确定要恢复所有调整比例为默认值吗？这将重置所有已调整的申报比例。",
    "确认恢复默认",
    {
    ...
    .then(() => {
      // 重置所有编辑状态为false
      editingStates.value = editingStates.value.map(() => false);

      // 创建新的ratios数组，设置为原始申报比例值
      const newRatios = props.tableData.map((row) => row.ratio);

      // 更新ratios
      emit("update:ratios", newRatios);

      // 显示成功消息
      ElMessage.success("已恢复所有调整比例为默认申报比例");
    })
    .catch(() => {
      // 用户取消操作
      console.log("用户取消了恢复默认调整比例操作");
    });
};
...
.strategy-table-container {
  ...
  .adjust-ratio-cell {
    ...
    .edit-btn {
      .el-icon {
        color: #409eff; /* Element Plus 主蓝色 - 可用状态 */
      }

      &.edit-btn-disabled {
        .el-icon {
          color: #c0c4cc !important; /* Element Plus 禁用灰色 */
        }
