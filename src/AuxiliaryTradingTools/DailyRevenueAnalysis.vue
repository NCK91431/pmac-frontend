<template>
  ...
  <div class="header-wrapper">
    <div class="header">
      <div class="header-content">
        <div class="title">⚡ 每日收益分析</div>
      </div>

      <div class="subtitle">
        查看每日收益分析，包括收益金额、收益来源、收益时间等信息
      </div>
    </div>
    <!-- 日期选择器区域 -->
  ...
  <div class="main-container">
    <!---------------------------- 左侧板块：三大图表显示区---------------------------- -->
    <div class="left-main-container" v-if="drawer">
      <!-- 天气信息 -->
      ...
      <template v-if="hasHistoryFlag">
        ...
        <div class="config-container">
          <div class="config-header">
            <div class="title-text">🔨 参数配置</div>
            <el-button type="primary" @click="toggleDrawer">
              {{ drawer ? "关闭天气" : "查看天气" }}</el-button
            >
          </div>
          <!-- 申报参数配置展示组件 -->
          <ConfigDisplay
        ...
        <div class="table-container">
          <div v-if="revenueLoading" class="strategy-loading">
            <div class="spinner"></div>
            <span>⏳ 正在加载收益表格数据...</span>
          </div>
          <template v-else>
            <div v-if="revenueError" class="no-data-hint">
              <span class="hint-icon">📊</span>
              <p>{{ revenueError }}</p>
            </div>
            <AnalysisTable v-else :tableData="revenueData" />
          </template>
        </div>
        <!-- 下面：Excel报表链接下载区 -->
        <div class="result-section">
          <div class="result-area">
            ...
            <!-- 加载状态 -->
            <div
              v-if="excelLinkLoading || revenueLoading"
              class="strategy-loading"
            >
              <div class="spinner"></div>
              <span>⏳ 正在生成收益分析Excel表...</span>
            </div>
            <!-- 加载结束 -->
            <div v-if="!excelLinkLoading && !revenueLoading">
              <div v-if="excelLinkError" class="error-box">
                ⚠️ 未返回文件下载链接，请检查日志
              </div>
              <div v-else class="download-link">
                <div>
                  📄 <strong>Excel 日前需求申报表</strong><br />
                  <span class="link-url">{{ revenueFileUrl }}</span>
                </div>
                <div>
                  <button
  ...
  <footer>🌟 基于EMS日收入分析 | 文件链接有效期24小时</footer>
</template>
...
const revenueFileUrl = ref("");
const revenueFileBlob = ref(null);
const excelLinkLoading = ref(false);
...
async function fetchHistoryRecord(date) {
  try {
    historyError.value = "";
    const response = await request.get(`/api/daily-declaration/history`, {
      params: { date },
    });
...
function buildRevenueParams() {
  ...
  const hd = historyData.value;
  const modeDeclMap = {
    manual: "手动",
    algorithm: "算法预测",
  };
  const modeDecl = hd.probability_window?.mode_declaration
    ? modeDeclMap[hd.probability_window.mode_declaration] ||
      hd.probability_window.mode_declaration
    : "算法预测";
  const ratio = hd.ratio_settings
    ? hd.ratio_settings.map((r) =>
        r !== null && r !== undefined ? parseFloat(r) : null,
      )
    : Array(24).fill(null);
  const probWindow = hd.probability_window
    ? {
        type: hd.probability_window.type || "all",
        days: hd.probability_window.days || 7,
        ...(hd.probability_window.mode_declaration
          ? { mode_declaration: hd.probability_window.mode_declaration }
          : {}),
        ...(hd.probability_window.period
          ? { period: hd.probability_window.period }
          : {}),
      }
    : { type: "all", days: 7 };
  return {
    declaration_date: hd.declaration_date
      ? new Date(hd.declaration_date).toLocaleDateString("en-CA")
      : "",
    load_mode: hd.load_mode || "算法预测",
    mode_declaration: modeDecl,
    probability_window: probWindow,
    ratio,
  };
}
...
async function fetchRevenueData() {
  const params = buildRevenueParams();
  if (!params) return;
  try {
    revenueLoading.value = true;
    revenueError.value = "";
    const response = await request.post("/api/revenue/data", params);
    if (
      response.data &&
      ...
      await fetchRevenueExcel();
    } else {
      revenueLoading.value = false;
      revenueData.value = [];
      revenueError.value = "收益数据格式异常";
    }
  } catch (error) {
    revenueLoading.value = false;
    console.error("获取收益数据失败:", error);
    revenueError.value = error.message || "获取收益数据失败";
    revenueData.value = [];
  }
}

const excelLinkError = ref("");
async function fetchRevenueExcel() {
  const params = buildRevenueParams();
  if (!params) return;
  excelLinkLoading.value = true;
  try {
    const response = await request.post("/api/revenue/generate", params, {
      responseType: "blob",
    });
    const blob = new Blob([response.data], {
      type:
    ...
    revenueFileBlob.value = blob;
    const blobUrl = URL.createObjectURL(blob);
    revenueFileUrl.value = blobUrl;
    excelLinkLoading.value = false;
    excelLinkError.value = "";
  } catch (error) {
    console.error("获取收益报表失败:", error);
    revenueFileUrl.value = "";
    revenueFileBlob.value = null;
    excelLinkLoading.value = false;
    excelLinkError.value = error.message || "获取收益报表失败";
  }
}

function handleDownloadExcel() {
  if (!revenueFileBlob.value) {
    ElMessage.warning("报表文件未生成，请先获取数据");
    return;
  }
  const fileName = `日前申报表_${historyData.value?.declaration_date?.slice(0, 10) || "unknown"}.xlsx`;
  const link = document.createElement("a");
  link.href = revenueFileUrl.value;
...
function handleCopyExcelLink() {
  if (!revenueFileUrl.value) {
    ElMessage.warning("链接为空，无法复制");
    return;
  }
  navigator.clipboard
    .writeText(revenueFileUrl.value)
    .then(() => {
      ElMessage.success("链接已复制到剪贴板");
    })
    .catch(() => {
