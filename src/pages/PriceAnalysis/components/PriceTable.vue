...
const tableData = computed(() => {
  ...
  if (props.timeType === "compute") {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        const index = i * 4 + j;
        realTimeSum += props.realTimeData[index] || 0;
        dayAheadSum += props.dayAheadData[index] || 0;
      }

      const realTimePrice = realTimeSum / 4;
      const dayAheadPrice = dayAheadSum / 4;
...
const averageRealTime = computed(() => {
  if (props.timeType === "compute") {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        const index = i * 4 + j;
        hourSum += props.realTimeData[index] || 0;
      }
      sum += hourSum / 4;
    }
    return sum / 24;
...
const averageDayAhead = computed(() => {
  if (props.timeType === "compute") {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        const index = i * 4 + j;
        hourSum += props.dayAheadData[index] || 0;
      }
      sum += hourSum / 4;
    }
    return sum / 24;
