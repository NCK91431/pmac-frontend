      ...
      <div class="header-controls">
        ...
        <div class="control-group">
          ...
          <div class="quick-btns">
            <button
              v-for="d in [7, 15, 30]"
              :key="d"
              :class="{ active: quickDays === d && !dateRange }"
              @click="handleQuick(d)"
            >
              近{{ d }}天
            </button>
        ...
        <div class="control-group">
          ...
          <div class="price-type-btns">
            <button
              :class="{ active: priceType === 'dayAhead' }"
              @click="switchType('dayAhead')"
            >
              日前结算
            </button>
            <button
              :class="{ active: priceType === 'realtime' }"
              @click="switchType('realtime')"
            >
              实时结算
            </button>
          </div>
      ...
      <template v-else>
        ...
        <div class="summary-cards">
          ...
          >
            ...
            <div class="card-value">
              {{ card.value }}<span class="card-unit">元/MWh</span>
            </div>
          </div>
        </div>
        ...
        >
          <div class="section-title">
            ...
            <span class="badge" :class="`badge-${group.type}`"
              >{{ group.slots.length }}个时刻</span
            >
          </div>
...
const config = computed(() =>
  isSharp.value ? TOU_CONFIG.sharp : TOU_CONFIG.normal,
);
const groupOrder = computed(() =>
  isSharp.value
    ? ["sharp", "peak", "normal", "valley"]
    : ["peak", "normal", "valley"],
);
...
function getDateRange() {
  ...
  const startStr = fmtDate(
    new Date(
      new Date(endStr + "T00:00:00").getTime() -
        (quickDays.value - 1) * 86400000,
    ),
  );
...
const dateLabels = computed(() =>
  (trendData.value?.dates || []).map((d) => d.slice(5)),
); // MM-DD
...
const hourly = computed(() => {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    result[key] = {
      type,
      prices: m.map((row) => (Array.isArray(row) ? row[h] : null)),
    };
  }
  return result;
});
...
const mainAvgText = computed(() =>
  avgAll.value == null ? "--" : avgAll.value.toFixed(2),
);
...
const summaryCards = computed(() => {
  ...
  cards.push({
    key: "avg",
    label: "区间日均价",
    value: avgAll.value == null ? "--" : avgAll.value.toFixed(2),
  });
  for (const type of groupOrder.value) {
    ...
    cards.push({
      key: type,
      label: `${TYPE_META[type].label}时段均价`,
...
function renderMainChart() {
  ...
  const maxVal = valid.length ? round2(Math.max(...valid)) : null;
  const minVal = valid.length ? round2(Math.min(...valid)) : null;
  const avgVal = avgOf(valid);

  // Y 轴上下限：向下/向上取整到百位，用于刻度红色上限 / 绿色下限标注
  ...
  const option = {
    ...
    yAxis: {
      ...
      axisLabel: {
        fontSize: 14,
        formatter(value) {
          if (Math.abs(value - yUpper) < matchThreshold)
            return "{max|" + value + "}";
          if (Math.abs(value - yLower) < matchThreshold)
            return "{min|" + value + "}";
          return "{normal|" + value + "}";
        },
        rich: {
...
.control-group :deep(.el-date-editor:hover) {
  border-color: #53c1de;
}
...
.control-group :deep(.el-date-editor .el-range-input::placeholder) {
  color: rgba(255, 255, 255, 0.45);
}
.control-group :deep(.el-date-editor .el-range-separator) {
  color: rgba(255, 255, 255, 0.5);
  font-size: 13px;
}
.control-group :deep(.el-date-editor .el-range__icon),
.control-group :deep(.el-date-editor .el-range__close-icon) {
...
.price-type-btns button:hover {
  border-color: #53c1de;
  color: #53c1de;
}
.quick-btns button.active,
.price-type-btns button.active {
  background: #53c1de;
  color: #1a1a2e;
  border-color: #53c1de;
  font-weight: 600;
...
.card-sharp .card-value {
  color: #a93226;
}
.card-peak .card-value {
  color: #bf4f00;
}
.card-normal .card-value {
  color: #1f6391;
}
.card-valley .card-value {
  color: #1e8449;
}
.card-avg .card-value {
  color: #8e44ad;
