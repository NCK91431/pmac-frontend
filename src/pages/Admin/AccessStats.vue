      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-controls">
            ...
            />
            <el-button type="primary" size="small" @click="handleFileSearch"
              >搜索</el-button
            >
          </div>
        </div>
        ...
        <el-card shadow="never">
          ...
          <el-table :data="changeDetailData" border stripe size="small">
            ...
            <el-table-column prop="changeValue" label="增减值">
              <template #default="{ row }">
                <span
                  :class="row.changeValue >= 0 ? 'text-success' : 'text-danger'"
                >
            ...
            <el-table-column prop="changePercent" label="增减百分比">
              <template #default="{ row }">
                <el-tag
                  :type="row.changePercent >= 0 ? 'success' : 'danger'"
                  size="small"
                >
                  {{ row.changePercent >= 0 ? "+" : ""
                  }}{{ (row.changePercent * 100).toFixed(1) }}%
                </el-tag>
              </template>
            </el-table-column>
          </el-table>
...
const loadModuleTrend = async () => {
  ...
  try {
    ...
    moduleTableData.value = Object.entries(moduleTotals).map(
      ([mod, visits]) => ({
        module: mod,
  ...
  try {
    ...
    if (error.response?.data instanceof Blob) {
      ...
      try {
        ...
        ElMessage.error("下载失败: " + (json.error || json.message));
      } catch {
